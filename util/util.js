 ...
 */
export function replaceAll(corpus, replacements, global = true) {
    let current = corpus;
...
export function getDayRoot(day, year, rootDir = path.join(getAppRoot(), "years")) {
    const dayWithLeadingZeros = String(day).padStart(2, "0");
    return path.join(rootDir, String(year), dayWithLeadingZeros);
}
export function getProblemUrl(day, year) {
    return `https://adventofcode.com/${year}/day/${day}`;
}
export async function getInput(day, year, rootDir = path.join(getAppRoot(), "years")) {
