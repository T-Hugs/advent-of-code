...
async function getDayData(day, year, fail = false) {
...
function parseArgs() {
    ...
    const sessionTokenIndex = args.findIndex(a => a === "--session" || a === "--session-token" || a === "--sessionToken" || a === "--token" || a === "-t");
    ...
    let templatePath = templatePathIndex >= 0 ? args[templatePathIndex + 1] : path.join(getAppRoot(), "solutionTemplate.ts.dat");
    const origTemplatePath = templatePath;
    const compareWithIndex = args.findIndex(a => a === "--compare-with");
    let compareWithPath = compareWithIndex >= 0 ? args[compareWithIndex + 1] : path.join(getAppRoot(), "compareTemplate.dat");
