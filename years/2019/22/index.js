...
export function linearDeal(pos, increment, deckSize) {
    return util.mod(pos * increment, deckSize);
}
export function linearStack(pos, deckSize) {
    return (pos - deckSize) * -1n - 1n;
}
export function reverseCut(pos, cutPos, deckSize) {
    return util.mod(pos + cutPos + deckSize, deckSize);
}
export function reverseDeal(pos, increment, deckSize) {
    return util.mod(util.modInverse(increment, deckSize) * pos, deckSize);
}
export function reverseStack(pos, deckSize) {
    return deckSize - pos - 1n;
}
async function p2019day22_part1(input) {
    // Linear solution
...
async function p2019day22_part2(input, part1Solution) {
    const deckSize = 119315717514047n;
    const numShuffles = 101741582076661n;
    const lines = input.split("\n");
    lines.reverse();
    function reverseShuffle(i, lines) {
        for (const line of lines) {
            if (line === "deal into new stack") {
    ...
    const A = util.mod((firstShuffle - secondShuffle) * util.modInverse(targetPos - firstShuffle + deckSize, deckSize), deckSize);
    const B = util.mod(firstShuffle - A * targetPos, deckSize);
    return util.mod(util.powerMod(A, numShuffles, deckSize) * targetPos +
        (util.powerMod(A, numShuffles, deckSize) - 1n) * util.modInverse(A - 1n, deckSize) * B, deckSize);
}
async function run() {
