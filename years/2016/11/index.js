...
async function p2016day11_part1(input) {
    return "Not implemented";
}
async function p2016day11_part2(input) {
    return "Not implemented";
}
async function run() {
    const part1tests = [];
    ...
    for (const testCase of part2tests) {
        test.logTestResult(testCase, String(await p2016day11_part2(testCase.input)));
    }
    test.endTests();
    // Get input and run program
    const input = await util.getInput(DAY, YEAR);
    const part1Solution = String(await p2016day11_part1(input));
    const part2Solution = String(await p2016day11_part2(input));
